...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        x = torch.relu(x)
        mu =3.5* torch.sigmoid(self.fc1(x))
        return mu
        # x = F.relu(self.fc1(x))
        ...
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
                ...
                if (i_episode + 1) % 10 == 0:
                    ...
                    # if i_episode % 50 == 0 and i_episode > 100:  # 保存神经网络参数
                save_data = {'net': agent.actor.state_dict(), 'opt': agent.actor_optimizer.state_dict(), 'i': i_episode}
                torch.save(save_data, "E:\GAIL_PP02_model_actor.pth")
                save_data = {'net': agent.critic.state_dict(), 'opt': agent.critic_optimizer.state_dict(),
                             'i': i_episode}
                torch.save(save_data, "E:\GAIL_PP02_critic.pth")
                pbar.update(1)
    return return_list
